...
import { gestureRuntimeBindingsFixture } from '../../../handlers/__tests__/gesture-runtime-bindings.fixtures.ts';
...
async function runDrag(sessionStore: ReturnType<typeof makeSessionStore>, sessionName: string) {
  return await handleInteractionCommands({
    ...
    sessionName,
    sessionStore,
    captureSnapshotForSession: async (session) => session.snapshot!,
    contextFromFlags,
