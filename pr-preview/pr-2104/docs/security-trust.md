...
If a hook is configured and its result does not attest a `tenantId`, the daemon never falls back to a tenant the client declares itself (RPC body `meta.tenantId` or `flags.tenant`, or the `x-agent-device-tenant` header on the upload/artifact-download/diagnostics routes): a request that also declares a tenant through any of those is refused (401) rather than run as whichever tenant the caller claimed, while a request that declares none proceeds unscoped — a shared token must not let one caller claim another tenant's identity. A hook that needs per-tenant scoping must attest `tenantId` explicitly; a deployment with no hook configured is unaffected.
